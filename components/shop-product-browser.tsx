...
const ShopGridProductCard = ({
  ...
  return (
    <article
      className="group flex h-full cursor-pointer flex-col overflow-hidden rounded-[10px] border border-black/10 bg-white transition hover:-translate-y-1 hover:shadow-[0_18px_45px_rgba(17,17,17,0.08)]"
      onClick={() => router.push(productHref)}
      onFocus={handlePrefetch}
      onMouseEnter={handlePrefetch}
    >
      <div className="relative aspect-[1.05] rounded-[12px] bg-[#f7f7f7]">
        {discount ? (
          <span className="absolute left-4 top-4 z-10 rounded bg-[#ef1d1d] px-3 py-1 text-xs font-bold text-white">
            -{discount}%
          </span>
        ) : isNew ? (
          <span className="absolute left-4 top-4 z-10 rounded bg-[#60ad4f] px-3 py-1 text-xs font-bold text-white">
            NEW
          </span>
        ) : null}
        <button
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mt-3 flex flex-wrap items-baseline gap-x-3 gap-y-1">
          <span
            className={cn(
              "max-w-full break-words text-[1.1rem] font-bold leading-none",
          ...
          {discount ? (
            <span className="max-w-full break-words text-sm text-gray-400 line-through">
        ...
        <button
          className="mt-auto inline-flex h-10 w-full items-center justify-center gap-2 rounded-md bg-[#050505] text-sm font-semibold text-white transition hover:bg-[#222222]"
          onClick={onAddToCart}
          type="button"
        >
...
const ShopProductBrowser: React.FC<ShopProductBrowserProps> = ({
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[300px_minmax(0,1fr)]">
      <aside className="rounded-[10px] border border-black/10 bg-white p-6 lg:sticky lg:top-6 lg:self-start">
        <div className="flex items-center justify-between border-b border-black/10 pb-5">
          <h2 className="text-2xl font-bold text-[#111111]">Filter By</h2>
          <button
            ...
            <div className="flex items-center gap-2">
              {[
                ...
                return (
                  <button
                    className={cn(
                      "inline-flex h-10 w-10 items-center justify-center rounded-md border border-black/10 transition",
                      viewMode === item.value
